...
const VideoPlayer2 = ({ src, fallbackSrc }: VideoPlayerProps) => {
  ...
  const idRef = useRef<string>(
    `bmvid_${Math.random().toString(36).slice(2, 9)}`
  );
  ...
  const broadcastUnmute = () => {
    const event = new CustomEvent("bettermarque-video-unmute", {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "bettermarque-video-unmute",
      onExternalUnmute as EventListener
    );
