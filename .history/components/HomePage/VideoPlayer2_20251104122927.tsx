...
const VideoPlayer2 = ({ src, fallbackSrc }: VideoPlayerProps) => {
  ...
  const idRef = useRef<string>(`bmvid_${Math.random().toString(36).slice(2, 9)}`);
  ...
  // When this player un-mutes, inform others so they mute themselves
  const broadcastUnmute = () => {
    const event = new CustomEvent("bettermarque-video-unmute", { detail: { id: idRef.current } });
  ...
  const toggleMute = () => {
    ...
    // If we're unmuting, tell other players to mute first
    if (v.muted) {
      broadcastUnmute();
      v.muted = false;
      setIsMuted(false);
  ...
  // attach native event listeners + fallback timer and "unmute" event listener
  useEffect(() => {
    const v = videoRef.current;
    if (!v) return;
    ...
    // When another player un-mutes, this handler will mute this player if it's not the origin
    const onExternalUnmute = (e: Event) => {
      try {
        const custom = e as CustomEvent<{ id: string }>;
        const originId = custom.detail?.id;
        if (originId && originId !== idRef.current) {
          // mute self
    ...
    window.addEventListener("bettermarque-video-unmute", onExternalUnmute as EventListener);

    // fallback: avoid spinner stuck case
    loaderTimeoutRef.current = window.setTimeout(() => {
      console.warn("[video] loader timeout reached — hiding spinner as fallback");
      markAsLoaded();
    }, 8000);

    // ensure local state matches video attributes
    setIsMuted(Boolean(v.muted));
    setIsPlaying(!v.paused);
    ...
    return () => {
      ...
      v.removeEventListener("pause", onPause);
      v.removeEventListener("volumechange", onVolumeChange);
      window.removeEventListener("bettermarque-video-unmute", onExternalUnmute as EventListener);
      if (loaderTimeoutRef.current) {
        window.clearTimeout(loaderTimeoutRef.current);
      }
    };
