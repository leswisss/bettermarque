...
const VideoPlayer = ({ src, fallbackSrc }: VideoPlayerProps) => {
  ...
  const cursorText = isPlaying ? "Pause Video" : "Play Video"
  const pauseplayRef = useRef(nu)
    ...
    <section className={`section ${styles.video__section}`}>
      <div className={`container ${styles.video__container}`}>
        <div className={styles.videoContainer} ref={containerRef}>
          ...
          <div className={styles.playPauseFill} onClick={togglePlay} ref></div>
          <Cursor text={cursorText} boundary={videoRef} activeState={cursor}/>
        </div>
      </div>
    </section>
  );
