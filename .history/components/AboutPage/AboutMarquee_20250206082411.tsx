  ...
  const animation = () => {
    ...
    if (imageWrapperRef1.current) {
      gsap.set(imageWrapperRef1.current, {
        rotation: xPercent * rotationSpeedRef.current,
  ...
  useGSAP(() => {
    ...
    gsap.to(sliderRef.current, {
      scrollTrigger: {
        ...
        scrub: 1,
      onUpdate: (e) => {// Determine scroll direction and adjust rotation speed
        const newDirection = Math.sign(e.direction); // Get scroll direction (+1 or -1)
        if (newDirection !== directionRef.current) {
          directionRef.current = newDirection; // Update direction
          rotationSpeedRef.current = newDirection * rotationSpeed; // Adjust rotation speed} // Dynamically change rotation speed), // Update directionRef's current property
      },
      x: "-150px",
    });
    requestAnimationFrame(animation);
