        ...
        <div className={styles.text__wrapper} ref={ref}>
          {splitWords(textone).map((word, i) => (
            <p className={styles.wword__wrapper} key={i}>
              <span className={styles.ww__span} style={{ transform: inView ? "translateY(0%)" : "translateY(100%)" }}>{word}</span>
            </p>
          ))}
        </div>
        {texttwo && (
          <div className={styles.text__wrapper} ref={ref2}>
            {splitWords(texttwo).map((word, i) => (
              <p className={styles.wword__wrapper} key={i}>
                <span className={styles.ww__span} style={{ transform: inView2 ? "translateY(0%)" : "translateY(100%)" }}>{word}</span>
              </p>
            ))}
          </div>
        )}
