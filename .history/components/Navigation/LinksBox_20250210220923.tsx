...
const LinksBox = ({content, target}: {content: FooterLinks, target}) => {
      ...
      <div className={styles.links__wrapper}>
        {
          content.links.map((data, i) => (
            <Link href={data.link} className={`${styles.link} ${data.link === pathname ? styles.translucent : ""}`} key={i}>{data.name}</Link>
          ))
        }
      </div>
    </div>
