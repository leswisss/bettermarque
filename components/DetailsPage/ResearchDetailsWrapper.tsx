...
const ResearchDetailsWrapper = () => {
  ...
  const researchParams = useParams();
  const researchSlug = researchParams.researchh;

  //Filter out current Project using its slug
  const currentProject = researchSlug
    ? ResearchContent.find(
        (project) => project.projectBox.slug === researchSlug
      )
    : null;
  ...
  //   ? ResearchContent[
  //       (ResearchContent.indexOf(currentProject) + 1) % ResearchContent.length
  //     ]
  //   : null;
  ...
  //     link: `/research/${nextProject?.projectBox.slug}`,
  ...
  return (
    <>
      ...
      >
        ...
        {currentProject.preImages3 &&
          currentProject.secondOtherDetail &&
          currentProject.videoLink2 && (
            <>
              <OtherDetails content={currentProject.secondOtherDetail} />{" "}
              <VideoPlayer2
                src={currentProject.videoLink2}
                fallbackSrc={currentProject.fallbackImage2}
              />
...
export default ResearchDetailsWrapper;
